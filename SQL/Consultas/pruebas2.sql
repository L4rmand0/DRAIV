select driver_information.dni_id,
            ...
            company.Name_company as company from `driver_information` 
				...
				inner join `admin3` on `admin3`.`adm3_id` = `driver_information`.`city_born` 
				where `driver_information`.`company_id` = ? and `driver_information`.`operation` != ? order by `driver_information`.`date_operation` desc